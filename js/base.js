...
jzm.find_unpaid_order = function()//查找未支付订单详情
{
    ...
    if (orderid) {
        ...
        .done(function(reg) {
            if(typeof(reg.statusCode) != 'undefined'){
            	jzm.Error(reg);
            }
            var str = "";
    ...
    {
        alert("Error！orderID is NULL!");
        window.location.href = "../index.html";
    };
};
//余额支付，微信支付
jzm.payMoney = function(){
    var orderid = jzm.getQueryString("orderId");
    var method = $("#paymethod").val();
    //优惠卷是否存在
    var rCouponIds = null,couponId = null;
    if($("#rCouponId").val() != ""){
        if($("#rCouponId").val() == 0){
            rCouponIds = null;
        }else{
    ...
    if (method == 0)    //余额支付
        {
            var paymoney = parseInt($("#totalMoney").text()),couponMoney = $("#couponMoney").val(),paypwd = 123456;
            $.ajax({
...
jzm.user_info = function(){
    ...
    .done(function(reg) {
        ...
        var time = jzm.getDateTime(reg.userInfo.loginTime);
        var str = "";
        ...
        str += '</ul></div>';
        str += '<div class="Registration" style="text-align: right; margin-top: 4%;" >';
            if(reg.signInfo != null){
                var nowDate = new Date().Format("yyyy/MM/dd");
                var oldTime = jzm.getDateTime(reg.signInfo.lastDate);
                if(nowDate == oldTime.split(' ')[0]){
                        str +='<button>已连续签到<br >'+ reg.signInfo.continuityCount +'天</button>';
                    }
                else{
                        str +='<button onclick="jzm.sign_in();">签到</button>';
                   };
                }
            else{
                    str +='<button onclick="jzm.sign_in();">签到</button>';
...
jzm.find_order_detail = function(e){
    if(!e){
        $.ajax({
            ...
            type: 'GET',
            dataType: 'json',
            data: {userId: us_userid,orderId:jzm.getQueryString("orderId"),userToken:us_token}
        })
        .done(function(reg) {
            if (typeof(reg.statusCode) != "undefined"){
                jzm.Error(reg);
            ...
            str += '<div style="flex: 1; height: 100%;margin-top:5%;text-align:center;"><img src="'+ reg.productInfo.productPicurl +'" alt="" style="width: 90%;"></div>'+
                        '<div  class="message" style="color:#828282;padding-top:3%;">'+
                            ...
                            '<p style="display: flex; font-size:1rem; "><span style="flex: 1; text-align: left; margin-right:2%;">时间</span><span style="flex: 2;text-align:right;margin-right:8%;">' + jzm.getDateTime(reg.orderInfo.createTime).split(' ')[0] +'</span></p>';
            for(var key in obj){
            ...
            liststr += '<li style="padding:3.7%;background:rgba(0,0,0,0.5);border-bottom:1px solid #828282;">'+
                    ...
                    '<li style="padding:3.7%;background:rgba(0,0,0,0.5);border-bottom:1px solid #828282;">'+
                        '<span>支付信息</span><span style=" text-align: right;color:#8c8c8c;">'+ jzm.getDateTime(reg.orderInfo.paymentTime).split(' ')[0] +'</span>'+
                    '</li>'+
                    '<li style="padding:3.7%;background:rgba(0,0,0,0.5);border-bottom:1px solid #828282;">'+
...
jzm.find_balance = function(){
    ...
    .done(function(reg) {
    	stateCode.test(reg.statusCode.status) ? jzm.Error(reg) : $("#balance").text("￥" + parseFloat((reg.balance != null ? reg.balance : 0) / 100).toFixed(2));
    })
};
//账户充值
jzm.WeChat_recharge = function(){
        ...
        .done(function(reg) {
        	jzm.recharge = function (){
                ...
                WeixinJSBridge.invoke(
                   ...
                   function(res){
                       if(res.err_msg == "get_brand_wcpay_request:ok" ) {
                        	window.location.href = "/sharedcoffee/html/my_wallet.html";
                       } // 使用以上方式判断前端返回,微信团队郑重提示：res.err_msg将在用户支付成功后返回    ok，但并不保证它绝对可靠。
                   }
               );
            }
            var rechargePay = [];
            stateCode.test(reg.statusCode.status) ? jzm.Error(reg) : void function(){
...
//消费明细
jzm.find_consumption_info = function(){
    $.ajax({
        ...
        for(var i = 0; i < reg.list.length; i++){
            ...
            str +='<li style="height: 15%; display: flex; margin:1% 2%;background:rgba(0,0,0,0.5);color:#828282;">'+
                ...
                '<div style="flex: 4; padding:4% 2% 2% 2%; font-size: 1.5rem;">'+
                    ...
                    '<p>'+ jzm.getDateTime(reg.list[i].paymentTime).split(' ')[0] +'</p>'+
                '</div>'+
...
//积分兑换
jzm.integral_exchange_true = function(id,change_length){
...
//故障反馈
jzm.add_fault_feedback = function(){
    ...
    .done(function(reg) {
        alert(reg.statusCode.msg);
       	//window.location.href = '/sharedcoffee/html/my.html';
    })
};
